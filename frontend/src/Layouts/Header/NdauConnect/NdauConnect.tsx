    ...
    if (socket) {
      ...
      socket.on(
        ...
        ({ walletAddress: _walletAddress }) => {
          ...
          //add admin
          socket.on(
            "server-add_admin-fulfilled-website",
            ({ adminAddress }) => {
          ...
          socket.on(
            "server-delete_admin-denied-website",
            ({ adminAddress }) => {
              toast.warn(
                "Deleting Admin: " + adminAddress + " - Denied By User "
              );
            }
          );

          socket.on(
            "server-delete_admin-failed-website",
            ({ adminAddress }) => {
              toast.error(
          ...
          socket.on(
            "server-proposal_reject-denied-website",
            ({ proposalId }) => {
          ...
          socket.on(
            "server-proposal_reject-failed-website",
            ({ proposalId }) => {
