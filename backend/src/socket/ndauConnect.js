...
const { createAdmin, deleteAdmin } = require('../controllers/admins_controller');
...
module.exports = (_io) => {
  _io.on('connection', (socket) => {
    ...
    socket.on('app-ndau_connection-established-server', ({ website_socket_id, connection_type, wallet_address, app_socket_id }) => {
      console.log('app-ndau_connection-established-server');
      if (connection_type === 'wallet') {
        socket.to(website_socket_id).emit('server-ndau_connection-established-website', {
    ...
    socket.on('website-ndau_connection-established-server', (_result) => {
      let appSocketId = webSocket_To_AppSocket_Map.get(_result.website_socket_id);
      socket.to(appSocketId).emit('server-ndau_connection-established-app', _result);
    });

    ///////////////////////////////////// proposal rejectio
    socket.on('website-proposal_reject-request-server', ({ proposalIdToReject, website_socket_id, wallet_address }) => {
    ...
    socket.on('app-proposal_reject-denied-server', ({ proposalId, app_socket_id }) => {
      ...
      socket.to(webSiteSocketId).emit('server-proposal_reject-denied-website', { proposalId }); //emit to website, proposal reject denied
    });

    socket.on('app-proposal_reject-confirmed-server', ({ proposalId, app_socket_id }) => {
      ...
      async function rejectProposalAndNotify() {
        ...
        if (rejectProposalResponse.status) {
          socket.to(webSiteSocketId).emit('server-proposal_reject-fulfilled-website', { proposalId }); //emit to website, proposal successfully rejected
          socket.emit('server-proposal_reject-fulfilled-app'); //emit back to mobile too, proposal successfully rejected
        } else {
          socket.to(webSiteSocketId).emit('server-proposal_reject-failed-website', { proposalId }); //emit to website, proposal rejection failed
    ...
    socket.on('website-proposal_approve-request-server', ({ proposalIdToApprove, website_socket_id, wallet_address }) => {
      const appSocketId = webSocket_To_AppSocket_Map.get(website_socket_id);
    ...
    socket.on('app-proposal_approve-denied-server', ({ proposalId, app_socket_id }) => {
      const webSiteSocketId = appSocket_To_WebSocket_Map.get(app_socket_id);

      console.log(proposalId, 'app-proposal_approve-denied-server');
      socket.to(webSiteSocketId).emit('server-proposal_approve-denied-website', { proposalId }); //emit to website, proposal approval denied
    });

    socket.on('app-proposal_approve-confirmed-server', ({ proposalId, app_socket_id }) => {
      const webSiteSocketId = appSocket_To_WebSocket_Map.get(app_socket_id);
      ...
      async function approveProposalAndNotify() {
        ...
        if (approveProposalResponse.status) {
          socket.to(webSiteSocketId).emit('server-proposal_approve-fulfilled-website', {
            proposalId,
          ...
          socket.to(webSiteSocketId).emit('server-proposal_approve-failed-website', { proposalId }); //emit to website, proposal approve failed
    ...
    socket.on('website-create_vote-request-server', async ({ websiteSocketId, selectedVoteOptionId, walletAddress }) => {
    ...
    socket.on('app-create_vote-rejected-server', async ({ voting_option_id, proposal_heading, voting_option_heading, wallet_address, app_socket_id }) => {
      // need to change it's name to app-create_vote-fulfilled-server, but isn't working correctly.
      // recheck at end
    ...
    ///// feature proposal
    socket.on('website-feature_proposal-request-server', ({ proposalIdToFeature, website_socket_id }) => {
      console.log(proposalIdToFeature, 'website-feature_proposal-request-server');
    ...
    socket.on('app-feature_proposal-denied-server', ({ proposalId, app_socket_id }) => {
      const webSiteSocketId = appSocket_To_WebSocket_Map.get(app_socket_id);

      console.log(proposalId, 'app-feature_proposal-denied-server');
      socket.to(webSiteSocketId).emit('server-feature_proposal-denied-website', { proposalId }); //emit to website, proposal approval denied
    });

    socket.on('app-feature_proposal-confirmed-server', ({ proposalId, app_socket_id }) => {
      const webSiteSocketId = appSocket_To_WebSocket_Map.get(app_socket_id);
      ...
      async function featureProposalAndNotify() {
        ...
        if (featureProposalResponse.status) {
          socket.to(webSiteSocketId).emit('server-feature_proposal-fulfilled-website', {
            proposalId,
          ...
          socket.to(webSiteSocketId).emit('server-feature_proposal-failed-website', { proposalId }); //emit to website, proposal featured failed
    ...
    ///// add admin
    socket.on('website-add_admin-request-server', ({ adminAddress, website_socket_id }) => {
    ...
    socket.on('app-add_admin-denied-server', ({ adminAddress, app_socket_id }) => {
      const webSiteSocketId = appSocket_To_WebSocket_Map.get(app_socket_id);

      console.log(adminAddress, 'app-add_admin-denied-server');
      socket.to(webSiteSocketId).emit('server-add_admin-denied-website', { adminAddress }); //emit to website, proposal approval denied
    });

    socket.on('app-add_admin-confirmed-server', ({ adminAddress, app_socket_id }) => {
      const webSiteSocketId = appSocket_To_WebSocket_Map.get(app_socket_id);
      ...
      async function addAdminAndNotify() {
        ...
        if (addAdminResponse.status) {
          socket.to(webSiteSocketId).emit('server-add_admin-fulfilled-website', {
            adminAddress,
          }); //emit to website, proposal successfully features
          ...
          socket.to(webSiteSocketId).emit('server-add_admin-failed-website', { adminAddress }); //emit to website, proposal featured failed
    ...
    ///// delete admin
    socket.on('website-delete_admin-request-server', ({ adminAddress, website_socket_id }) => {
    ...
    socket.on('app-delete_admin-denied-server', ({ adminAddress, app_socket_id }) => {
      const webSiteSocketId = appSocket_To_WebSocket_Map.get(app_socket_id);

      console.log(adminAddress, 'app-delete_admin-denied-server');
      socket.to(webSiteSocketId).emit('server-delete_admin-denied-website', { adminAddress }); //emit to website, delete admin denied
    });

    socket.on('app-delete_admin-confirmed-server', ({ adminAddress, app_socket_id }) => {
      const webSiteSocketId = appSocket_To_WebSocket_Map.get(app_socket_id);
      ...
      async function deleteAdminAndNotify() {
        ...
        if (addAdminResponse.status) {
          socket.to(webSiteSocketId).emit('server-delete_admin-fulfilled-website', {
            adminAddress,
          }); //emit to website, proposal successfully features
          ...
          socket.to(webSiteSocketId).emit('server-delete_admin-failed-website', {
            adminAddress,
...
async function create_vote_request_func({ websiteSocketId, selectedVoteOptionId, walletAddress, socket: _socket }) {
  ...
  if (voteInfoForConfirmation.proposal_id || voteInfoForConfirmation.status !== false) {
    _socket.to(appSocketId).emit('server-create_vote-request-app', voteInfoForConfirmation);
  } else {
    _socket.to(appSocketId).emit('server-create_vote-failed-app', voteInfoForConfirmation);
